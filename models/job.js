...
class Job {
	...
	static async findAll(reqQueries) {
		let jobsRes;
		if (reqQueries && Object.keys(reqQueries).length > 0) {
			const { dbQuery, filterValues } = sqlForFilteringJobs(reqQueries);
			console.log("These are the sql filtered values", dbQuery, filterValues);
	 ...
	 */
	 ...
	 // Try to put the validation into the schema.
